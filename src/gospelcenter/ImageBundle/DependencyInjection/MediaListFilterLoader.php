...
namespace gospelcenter\ImageBundle\Imagine\Filter\Loader;
...
class MediaListFilterLoader implements LoaderInterface
{
    /**
     * {@inheritDoc}
     */
    public function load(ImageInterface $image, array $options = array())
    {
        ...
        if (($origWidth > $width || $origHeight > $height)
            ...
            $filter = new Thumbnail(new Box($width*2, $height*2), $mode, $filter);
            $image2 = $filter->apply($image);
        }

        return array($image, $image2);
    }
}
